...
import org.xper.robot.acq.counter.RobotMarkEveryStepExptSpikeCounter;
import org.xper.robot.drawing.preview.PNGmaker;
import org.xper.robot.drawing.stick.MStickSpec;
import org.xper.robot.drawing.stimuli.AldenSpec_class;
import org.xper.robot.drawing.stimuli.BlenderSpec;
import org.xper.robot.drawing.stimuli.EnviroSpec_class;
import org.xper.robot.drawing.stimuli.PngObjectSpec;
import org.xper.robot.expt.RobotExptSpec;
import org.xper.robot.expt.RobotExptSpecGenerator;
import org.xper.robot.parsedata.DataObject;
import org.xper.robot.util.BlenderRunnable;
import org.xper.robot.util.ExpLogMessage;
import org.xper.robot.util.RobotDbUtil;
import org.xper.robot.util.RobotIOUtil;
import org.xper.time.TimeUtil;


public class RobotRandomGeneration {
	@Dependency
	RobotDbUtil dbUtil;
	@Dependency
	...
	@Dependency
	RobotExptSpecGenerator generator;	
	@Dependency
	int taskCount;
...
char cont = 'y';  // 'n'; //
		...
		while (cont == 'y') {
			int c = RobotIOUtil.promptInteger("Enter GA run number to continue. Else enter '0'");
			System.out.println("");
			if (c==0) {
...
//					previously: 's' (stability), 'd' (density)
					progressType = RobotIOUtil.promptString("");
				...
				if (Arrays.asList("c","a","gs","p","m","o","cm","s","i","l","cc","y").contains(progressType)) {
					...
					String overrideAutoSelect = RobotIOUtil.promptString("");
					
					if (!overrideAutoSelect.equals("-")) {
						specifiedPostHocStimuli.add(overrideAutoSelect);
						
						while (!overrideAutoSelect.equals("-")) {
							overrideAutoSelect = RobotIOUtil.promptString("");
				...
				case "g":
					...
					break;
				case "gs":
					postHoc = "GRASSGRAVITYSTIMULUS";
					createPHgrassGravityWithStimulus();
					break;
				case "c":
					postHoc = "COMPOSITE";
...
//			cont = RobotIOUtil.prompt("Continue recording?");
			cont = 'n';
		}
...
//		}
	...
	void createPHcomposite(boolean cameraLocationChange) {
		...
		String specifyArchitecture = RobotIOUtil.promptString("Would you like to specify the architecture?");

		if (specifyArchitecture.equals("y")) {

			floor_decision_lin1 = RobotIOUtil.promptInteger("Floor?");
			ceiling_decision_lin1 = RobotIOUtil.promptInteger("Ceiling?");
			wallL_decision_lin1 = RobotIOUtil.promptInteger("Left wall?");
			wallR_decision_lin1 = RobotIOUtil.promptInteger("Right wall?");
			wallB_decision_lin1 = RobotIOUtil.promptInteger("Back wall?");
...
//		}
	...
	void createPHgrassGravityWithStimulus() {
		
		// same as grassGravity, but with single high stimulus instead of wooden ball and no objectless images
		List<Long> blankStimObjIds = new ArrayList<Long>();	
		List<Long> stimObjIds = new ArrayList<Long>();
		...
		//need to generate in same manner as library generation
		//need to somehow call matlab to fill out the background or place library image behind in imageStack
		
		BlenderRunnable blenderRunnerRefresh = new BlenderRunnable(PngGAParams.basePath + "grassGravityWithStimulusPostHoc.py");
		blenderRunnerRefresh.run();
...
//		}
	...
	void createPHoptics() {
		...
		for (int n=0;n<stimsToMorph_lin1.size();n++) {
			...
			for (int m=0;m<27+1;m++) {
				long whichStim_lin1 = generator.generatePHStimAnimacy(prefix, runNum, genNum, linNum, currentId, stimNum, "OPTICS");
				stimObjIds.add(whichStim_lin1);
				System.out.println("Lineage " + linNum + ": Generating and saving stimulus " + n + " number " + m);
				stimNum ++;
			}
		...
		BlenderRunnable blenderRunnerRefresh = new BlenderRunnable(PngGAParams.basePath + "opticsPostHoc.py");
		blenderRunnerRefresh.run();
...
//		}
	...
	void createPHmaterial() {
		...
		for (int n=0;n<stimsToMorph_lin1.size();n++) {
			...
			for (int m=0;m<30+1;m++) {
				long whichStim_lin1 = generator.generatePHStimAnimacy(prefix, runNum, genNum, linNum, currentId, stimNum, "MATERIAL");
				stimObjIds.add(whichStim_lin1);
				System.out.println("Lineage " + linNum + ": Generating and saving stimulus " + n + " number " + m);
				stimNum ++;
			}
		...
		BlenderRunnable blenderRunnerRefresh = new BlenderRunnable(PngGAParams.basePath + "cannedPostHoc.py");
		blenderRunnerRefresh.run();
...
//		}
	...
	void createGATrialsFromStimObjs(List<Long> stimObjIds) {
		...
		String finishTasks = RobotIOUtil.promptString("Wait for unfinished tasks to complete?");
...
//		RobotMarkEveryStepExptSpikeCounter spikeCounter = new RobotMarkEveryStepExptSpikeCounter(); 
...
//				
//				// get TrialSpec:
//				RobotExptSpec trialSpec = RobotExptSpec.fromXml(dbUtil.getSpecByTaskId(taskId).getSpec());
//				
...
//		}
	...
	public RobotExptSpecGenerator getGenerator() {
		return generator;
	}

	public void setGenerator(RobotExptSpecGenerator generator) {
	...
	//	RobotDbUtil dbUtil = new RobotDbUtil(ds);
	//	
	//	RobotRandomGeneration prg = new RobotRandomGeneration();
