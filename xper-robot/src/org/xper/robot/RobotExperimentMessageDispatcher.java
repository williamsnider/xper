...
import org.xper.robot.vo.RobotTargetMessage;
import org.xper.robot.vo.RobotTrialContext;
import org.xper.robot.vo.RobotTrialOutcomeMessage;

public class RobotExperimentMessageDispatcher extends
		TrialExperimentMessageDispatcher implements RobotTrialEventListener {

	public void targetOn(long timestamp, RobotTrialContext context) {
		enqueue(timestamp, "TargetOn", RobotTargetMessage
				.toXml(new RobotTargetMessage(timestamp, context.getTargetPos(), context.getTargetEyeWindowSize())));
	}
...
//	}
	...
	public void trialPASS(long timestamp, RobotTrialContext context) {
		...
		enqueue(timestamp,"TrialOutcome",RobotTrialOutcomeMessage
				.toXml(new RobotTrialOutcomeMessage(timestamp,"PASS",taskId,genId)));
//		enqueue(taskId,"TaskIdOutcome",RobotTrialOutcomeMessage
//				.toXml(new RobotTrialOutcomeMessage(timestamp,"PASS",taskId,genId)));		// hack so I can easily find outcome by taskId
	...
	public void trialFAIL(long timestamp, RobotTrialContext context) {
		...
		long genId = context.getCurrentTask().getGenId();
		enqueue(timestamp,"TrialOutcome",RobotTrialOutcomeMessage
				.toXml(new RobotTrialOutcomeMessage(timestamp,"FAIL",taskId,genId)));
//		enqueue(taskId,"TaskIdOutcome",RobotTrialOutcomeMessage
//				.toXml(new RobotTrialOutcomeMessage(timestamp,"FAIL",taskId,genId)));
	...
	public void trialBREAK(long timestamp, RobotTrialContext context) {
		...
		long genId = context.getCurrentTask().getGenId();
		enqueue(timestamp,"TrialOutcome",RobotTrialOutcomeMessage
				.toXml(new RobotTrialOutcomeMessage(timestamp,"BREAK",taskId,genId)));
//		enqueue(taskId,"TaskIdOutcome",RobotTrialOutcomeMessage
//				.toXml(new RobotTrialOutcomeMessage(timestamp,"BREAK",taskId,genId)));
	...
	public void trialNOGO(long timestamp, RobotTrialContext context) {
		...
		long genId = context.getCurrentTask().getGenId();
		enqueue(timestamp,"TrialOutcome",RobotTrialOutcomeMessage
				.toXml(new RobotTrialOutcomeMessage(timestamp,"NOGO",taskId,genId)));
//		enqueue(taskId,"TaskIdOutcome",RobotTrialOutcomeMessage					// can't do this or it will try to write this msg multiple times (when it re-runs any nogo stimuli) because it uses taskId instead of tstamp!
//				.toXml(new RobotTrialOutcomeMessage(timestamp,"NOGO",taskId,genId)));
	...
	public void initialEyeInFail(long timestamp, TrialContext context) {
		...
		trialNOGO(timestamp,(RobotTrialContext) context);
	}
