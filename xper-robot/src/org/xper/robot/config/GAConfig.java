...
import org.xper.robot.RobotTrialExperiment;
import org.xper.robot.expt.RobotExptScene;
...
//	}
	...
	@Bean
	public RobotTrialExperiment experiment() {
		RobotTrialExperiment xper = new RobotTrialExperiment();
		xper.setStateObject(experimentState());
		// JK share the dbUtil with the experiment
		xper.setDbUtil(generalConfig.robotDbUtil());
...
//		xper.setEarlyTargetFixationAllowableTime(0);	// do not allow eyemovements during last stimulus for GA
	...
	@Bean
	public RobotExptScene taskScene() {
		RobotExptScene scene = new RobotExptScene();
		...
		scene.setDbUtil(generalConfig.robotDbUtil());
...
//		scene.setBlankScreen(new PngBlankScreen());
	...
	@Bean
	public RobotExptSpecGenerator generator() {
		RobotExptSpecGenerator gen = new RobotExptSpecGenerator();
		gen.setDbUtil(generalConfig.robotDbUtil());
	...
	@Bean
	public RobotRandomGeneration randomGen() {
		RobotRandomGeneration gen = new RobotRandomGeneration();
		gen.setDbUtil(generalConfig.robotDbUtil());
		gen.setGlobalTimeUtil(acqConfig.timeClient());
		gen.setRenderer(generalConfig.experimentGLRenderer());
	...
	@Bean
	public RobotExperimentState experimentState() {
		RobotExperimentState state = new RobotExperimentState();
